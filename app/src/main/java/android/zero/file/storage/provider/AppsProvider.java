...
package android.zero.file.storage.provider;
...
import android.zero.file.storage.cursor.MatrixCursor;
import android.zero.file.storage.cursor.MatrixCursor.RowBuilder;
import android.zero.file.storage.misc.FileUtils;
import android.zero.file.storage.misc.PackageManagerUtils;
import android.zero.file.storage.misc.StorageUtils;
import android.zero.file.storage.misc.Utils;
import android.zero.file.storage.model.DocumentsContract;
import android.zero.file.storage.model.DocumentsContract.Document;
...
public class AppsProvider extends DocumentsProvider {
  ...
  private static SparseArray<String> zeroser;
  ...
  static {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
      // 对于 API 28 及以上版本，使用 IMPORTANCE_CACHED
      processTypeCache.put(ActivityManager.RunningAppProcessInfo.IMPORTANCE_CACHED, "Empty");
    } else {
      // 对于 API 28 以下版本，使用 IMPORTANCE_EMPTY
      //  processTypeCache.put(ActivityManager.RunningAppProcessInfo.IMPORTANCE_EMPTY, "Empty");
      // IMPORTANCE_EMPTYs(zeroser);
      IMPORTANCE_EMPTY();
    }
  }
  ...
  public static void IMPORTANCE_EMPTYs(SparseArray<String> cache) {
    cache.put(ActivityManager.RunningAppProcessInfo.IMPORTANCE_EMPTY, "Empty");
  }

  // 方案2
  @SuppressWarnings("deprecation")
  public static void IMPORTANCE_EMPTY() {
    processTypeCache = new SparseArray<String>();
    processTypeCache.put(ActivityManager.RunningAppProcessInfo.IMPORTANCE_EMPTY, "Empty");
  }
  ...
  */
  ...
  public static void notifyRootsChanged(Context context) {
    ...
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
      // 使用两参数版本的 notifyChange 方法
      contentResolver.notifyChange(rootsUri, null);
    } else {
      // 使用三参数版本的 notifyChange 方法（适用于旧版本）
      contentResolver.notifyChange(rootsUri, null, false);
    }
  }
  ...
  public Cursor queryRoots(String[] projection) throws FileNotFoundException {
    ...
    row.add(
        Root.COLUMN_AVAILABLE_BYTES,
        storageUtils.getPartionSize(StorageUtils.PARTITION_DATA, false));
    row.add(
        Root.COLUMN_CAPACITY_BYTES, storageUtils.getPartionSize(StorageUtils.PARTITION_DATA, true));
    ...
    row2.add(
        Root.COLUMN_AVAILABLE_BYTES,
        storageUtils.getPartionSize(StorageUtils.PARTITION_RAM, false));
    row2.add(
        Root.COLUMN_CAPACITY_BYTES, storageUtils.getPartionSize(StorageUtils.PARTITION_RAM, true));
  ...
  public String copyDocument(String sourceDocumentId, String targetParentDocumentId)
    ...
    try {
      ...
      fileName =
          (String)
              (appInfo.loadLabel(packageManager) != null
    ...
    if (!FileUtils.moveDocument(fileFrom, fileTo, fileName)) {
      ...
      FileUtils.updateMediaStore(
          getContext(),
          FileUtils.makeFilePath(
              fileTo.getPath(), fileName + "." + FileUtils.getExtFromFilename(fileFrom.getPath())));
    }
    return fromFilePath;
  }
    ...
    try {
      if (docId.startsWith(ROOT_ID_USER_APP)) {
        ...
        if (Utils.hasOreo()) {
          ...
          List<RunningAppProcessInfo> runningProcessesList =
  ...
  private void includeDefaultDocument(MatrixCursor result, String docId) {
    ...
    row.add(
        Document.COLUMN_FLAGS, Document.FLAG_SUPPORTS_THUMBNAIL | Document.FLAG_SUPPORTS_DELETE);
  }
  ...
  private void includeAppFromProcess(
      MatrixCursor result, String docId, RunningAppProcessInfo processInfo, String query) {
    if (isImportanceNotExcluded(processInfo)) {
      String process = processInfo.processName;
      process = process.substring(process.lastIndexOf(".") + 1, process.length());
      String summary = "";
      String displayName = "";
      ...
      try {
        appInfo =
            packageManager.getPackageInfo(processInfo.processName, PackageManager.GET_ACTIVITIES)
                .applicationInfo;
        displayName =
            (String)
                (appInfo.loadLabel(packageManager) != null
      ...
      if (!TextUtils.isEmpty(query) && !displayName.toLowerCase().contains(query)) {
        return;
      }
      final String path = appInfo != null ? appInfo.sourceDir : "";
      final String mimeType = Document.MIME_TYPE_APK;
  ...
  @SuppressWarnings("deprecation")
  private boolean isImportanceNotExcluded(RunningAppProcessInfo processInfo) {
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      // For Android 8.0 and above
      return processInfo.importance != RunningAppProcessInfo.IMPORTANCE_GONE;
    } else {
      // For Android versions below 8.0
      return processInfo.importance != RunningAppProcessInfo.IMPORTANCE_EMPTY
          && processInfo.importance != RunningAppProcessInfo.IMPORTANCE_PERCEPTIBLE;
    }
  }

  private void includeAppFromPackage(
      MatrixCursor result,
      String docId,
    ...
    if (showSystem == isSystemApp(appInfo)) {
      ...
      row.add(
          Document.COLUMN_DISPLAY_NAME,
          getAppName(displayName) + getAppVersion(packageInfo.versionName));
      row.add(Document.COLUMN_SUMMARY, summary);
      row.add(Document.COLUMN_SIZE, size);
      row.add(Document.COLUMN_MIME_TYPE, mimeType);
      row.add(Document.COLUMN_LAST_MODIFIED, lastModified);
  ...
  private void includeAppFromProcess(
      MatrixCursor result, String docId, AndroidAppProcess processInfo, String query) {
    ...
    try {
      appInfo =
          packageManager.getPackageInfo(packageName, PackageManager.GET_ACTIVITIES).applicationInfo;
      displayName = process;
    } catch (Exception e) {
    }
  ...
  private void includeAppFromService(
      MatrixCursor result, String docId, RunningServiceInfo processInfo, String query) {
...
*/
    ...
    private static int getAppListFlag() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            // 对于 Android Nougat (7.0) 及以上版本，使用 MATCH_UNINSTALLED_PACKAGES
            return PackageManager.MATCH_UNINSTALLED_PACKAGES;
        } else {
            // 对于 Android Nougat 以下版本，使用 GET_UNINSTALLED_PACKAGES
  ...
  private static boolean isSystemApp(ApplicationInfo appInfo) {
    return appInfo.flags != 0
        && (appInfo.flags & (ApplicationInfo.FLAG_UPDATED_SYSTEM_APP | ApplicationInfo.FLAG_SYSTEM))
            > 0;
  }
   ...
   *
   * @return a list of RunningAppProcessInfo records, or null if there are no running processes (it
