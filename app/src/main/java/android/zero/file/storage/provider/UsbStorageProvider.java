...
import android.zero.file.storage.misc.ParcelFileDescriptorUtil;
...
import android.zero.file.storage.usb.UsbUtils;
...
import static android.zero.file.storage.misc.FileUtils.getTypeForName;
import static android.zero.file.storage.misc.MimeTypes.BASIC_MIME_TYPE;
...
public class UsbStorageProvider extends DocumentsProvider {
     ...
     */
    private static final String ACTION_USB_PERMISSION = "android.zero.file.storage.action.USB_PERMISSION";
...
//    try {
//        // 使用libaums库发现USB设备
//        UsbMassStorageDevice storageDevice = UsbMassStorageDevice.getMassStorageDevices(context);
//        storageDevice.init();
...
//        mRoots.put(getRootId(usbDevice), usbPartition);
//    } catch (Exception e) {
//        Log.e("UsbStorageProvider", "Failed to discover USB device: " + e.getMessage());
//    }
//}
//
    private void discoverDevice(UsbDevice usbDevice) {
    try {
        ...
        if (storageDevices.length > 0) {
            ...
            Log.e("UsbStorageProvider", "No USB mass storage devices found");
        }
    } catch (Exception e) {
        Log.e("UsbStorageProvider", "Failed to discover USB device: " + e.getMessage());
    }
...
private void notifyRootsChanged() {
    context.getContentResolver().notifyChange(DocumentsContract.buildRootsUri(AUTHORITY), null, false);
}

private void notifyDocumentsChanged(Context context, String rootId) {
    context.getContentResolver().notifyChange(DocumentsContract.buildDocumentUri(AUTHORITY, rootId), null, false);
}
...
//        context.registerReceiver(mUsbReceiver, filter);
        // 注册BroadcastReceiver
        IntentFilter filter = new IntentFilter();
        filter.addAction(UsbStorageProvider.ACTION_USB_PERMISSION);
        filter.addAction(UsbManager.ACTION_USB_DEVICE_ATTACHED);
        filter.addAction(UsbManager.ACTION_USB_DEVICE_DETACHED);
       // getContext().registerReceiver(mUsbReceiver, filter, Context.RECEIVER_NOT_EXPORTED);
    ...
    private void notifyRootsChanged1() {
    ...
    public static void notifyDocumentsChanged1(Context context, String rootId) {
...
/*            final int accessMode = ParcelFileDescriptorUtil.parseMode(mode);
    ...
    private void discoverDevice1(UsbDevice device) {
...
public void onReceive(Context context, Intent intent) {
    ...
    if (UsbManager.ACTION_USB_DEVICE_ATTACHED.equals(action)) {
        ...
        if (usbDevice != null) {
            ...
            if (!usbManager.hasPermission(usbDevice)) {
                ...
                notifyRootsChanged();
                notifyDocumentsChanged(context, getRootId(usbDevice) + ROOT_SEPERATOR);
            }
        }
        ...
        if (permission && usbDevice != null) {
            discoverDevice(usbDevice);
            notifyRootsChanged();
            notifyDocumentsChanged(context, getRootId(usbDevice) + ROOT_SEPERATOR);
        } else {
            // 用户拒绝了权限请求，可以在这里处理
            Log.w("UsbStorageProvider", "USB permission denied for device: " + usbDevice);
        }
    }
  }
 };
    ...
    private String getRootId1(UsbDevice usbDevice) {
        return ROOT_ID_USB + Integer.toString(usbDevice.getDeviceId());
    }
