...
public class MaterialProgressBar extends ProgressBar {
  ...
  public MaterialProgressBar(Context context, AttributeSet attrs, int defStyle) {
    ...
    TypedArray a =
        context.obtainStyledAttributes(attrs, R.styleable.MaterialProgressBar, defStyle, 0);

    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
      final int color =
          a.getColor(
              R.styleable.MaterialProgressBar_color, ContextCompat.getColor(R.color.accentColor));
    } else {
      final int color =
          a.getColor(R.styleable.MaterialProgressBar_color, res.getColor(R.color.accentColor));
    }
    ...
    final float strokeWidth =
        a.getDimension(
            R.styleable.MaterialProgressBar_stroke_width,
            res.getDimension(R.dimen.default_stroke_width));
    final float sweepSpeed =
        a.getFloat(
            R.styleable.MaterialProgressBar_sweep_speed,
            Float.parseFloat(res.getString(R.string.default_sweep_speed)));
    final float rotationSpeed =
        a.getFloat(
            R.styleable.MaterialProgressBar_rotation_speed,
            Float.parseFloat(res.getString(R.string.default_rotation_speed)));
    final int minSweepAngle =
        a.getInteger(
            R.styleable.MaterialProgressBar_min_sweep_angle,
            res.getInteger(R.integer.default_min_sweep_angle));
    final int maxSweepAngle =
        a.getInteger(
            R.styleable.MaterialProgressBar_max_sweep_angle,
            res.getInteger(R.integer.default_max_sweep_angle));
    a.recycle();
